...
- Pass  domain_adapt=True  in config to attach a DomainClassifierHead
  behind a GradientReversalLayer on the CLS embedding.
- The domain head is exposed via  model.domain_head  and can be
  called separately:  logits = model.domain_head(embedding)
- Alpha is updated externally:  model.domain_head.set_alpha(alpha)
"""
...
# Import domain components (adjust path to wherever you place domain_components.py)
from .domain_components import DomainClassifierHead
...
class FlatTransformerV2(nn.Module):
    ...
    Args:
        config: Dictionary with:
            ...
            - domain_adapt: bool — attach domain classifier head (default: False)
            - domain_head_hidden_dim: hidden dim for domain head (default: d_model // 2)
    """
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # ── Domain adaptation head (optional) ──────────────────────────
        domain_adapt = config.get('domain_adapt', False)
        if domain_adapt:
            domain_hidden = config.get(
                'domain_head_hidden_dim', max(self.d_model // 2, 64)
            )
            self.domain_head: Optional[DomainClassifierHead] = DomainClassifierHead(
                embed_dim=self.d_model,
    ...
    def forward(
        ...
        Args:
            ...
            target:       (B, 3) unit vectors — required for vMF loss

        Returns:
            dict with keys:
                'direction'    : (B, 3) unit vectors
                'embedding'    : (B, d_model) CLS embedding
                'domain_logits': (B,) domain logits — only present when
                                  domain_head is attached AND model is in
                                  training mode
                'loss'         : vMF loss scalar — only present for vMF head
                                  with target supplied
        """
        ...
        if self.head_type == 'directional':
            ...
            out = {'direction': direction, 'embedding': embedding}
        else:
        ...
        # We only compute domain logits when the head is present.
        # During inference / validation the head is simply skipped.
        if self.domain_head is not None and self.training:
            out['domain_logits'] = self.domain_head(embedding)
